...
// Made refactoring in oreder to make code clear and readable
...
// Login block(open browser, get link, enter creditencials and login)
...
async function getWeather(driver, city) {
  await driver.get(`https://www.google.com/search?q=weather+${city}`)

  temperature = await getTemperature(driver)

  condition = await getCondition(driver)

  return `${city}: ${temperature}°C, ${condition}`
}

// Get temperature
async function getTemperature(driver) {
  let temperature = await driver
    .wait(until.elementLocated(By.id('wob_tm')))
    .getText()
...
// Get condition
async function getCondition(driver) {
  let condition = await driver
    .wait(until.elementLocated(By.id('wob_dc')))
    .getText()
...
async function main(driver) {
  ...
  let weatherResults = []
  for (let city of cities) {
    let weather = await getWeather(driver, city)
    weatherResults.push(weather)
  }
  console.log(weatherResults)
}
